...
@section('content')
    <div class="max-w-5xl mx-auto bg-white p-6 rounded-md mt-4 shadow-md">
        <div class="flex justify-between items-center pt-2 pb-4">
            <h1 class="text-lg font-semibold text-gray-700">Edit Data</h1>
        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input value="{{$post->title}}" type="text" id="title" name="title" class="@error('title') border-red-500 @enderror w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="Masukkan judul disini">
                @error('title')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="mb-4">
                <label for="content" class="block text-gray-700 text-sm font-semibold">Content:</label>
                <textarea id="content" name="content" rows="4" class="@error('content') border-red-500 @enderror w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="Masukkan konten disini">{{$post->content}}</textarea>
